...
var resetPassword = (username) => {
	...
	return db.knex('users').where({ email: email }).first().then((user) => {
		...
		if (!user) {
			var error = new Error(`Email '${email}' does not match an existing account. Please contact helyx.io.`);
			error.code = "UNKNOWN_EMAIL";
			error.reason = `Email '${email}' does not match an existing account. Please contact helyx.io.`;
			return Promise.reject(error);
		} else {
			...
			return db.knex('users').where({ email: email }).update(userDataToUpdate).then(() => {
				...
				var subject = "Helyx.io - Password reset";

				return emailService.sendEmail(email, user.givenName, subject, htmlBody, textBody, undefined, 'Helyx.io');
			});
...
var sendPasswordResetDemand =  (subject, resetToken, username) => {
	...
	return emailService.sendEmail(email, username, subject, html, text, undefined, 'Helyx.io');
};
